...
```{r}
for(pheno in c("neuros","collaterals","rank_transformation","brainomix")){
    sumstat <- fread(paste0("/home/stroke/Documentos/Miquel_Gallardo/real_data/data/GWAS/geno_assoc_",pheno,"_clean.gwas.gz"))
    sumstat <- as.data.frame(sumstat) # Necesario para colsplit
    sumstat[,((ncol(sumstat)+1):(ncol(sumstat)+3))] <- colsplit(sumstat$SNP,pattern = ":",c("rsid","aa","ab"))
    colnames(sumstat)[3] <- "BP"
    
    sumstat_nors <- sumstat[-grep("rs",sumstat$rsid),]
    sumstat <- sumstat[grep("rs",sumstat$rsid),]
    
    # A dataframe is needed that has the rs information stored in a specific column.
    sumstat_nors <- convert_loc_to_rs(sumstat_nors, dbSNP = SNPlocs.Hsapiens.dbSNP155.GRCh38)
    # Add rs column at the end of the dataframe 
    
    sumstat_nors$rsid <- sumstat_nors$SNP.x
    
    
    sumstat <- sumstat[,c("rsid","CHR","BP","A1","A2","BETA","SE")]
    sumstat_nors <- sumstat_nors[,c("rsid","CHR","BP","A1","A2","BETA","SE")]
    
    sumstat <- rbind(sumstat, sumstat_nors)
    
    sumstat <- sumstat[!is.na(sumstat$rsid),] # Remove values without rs
    sumstat$Z <- sumstat$BETA/sumstat$SE
    sumstat <- sumstat[,c("rsid","CHR","BP","A1","A2","Z")]
    colnames(sumstat)[1] <- "SNP"
    fwrite(sumstat, file=paste0("/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/data/sumstats",pheno,".sumstats.gz"), col.names = T, row.names = F, quote = F, sep = "\t")
}
```
...
# Parameters 1
```{r}
# repeat this chunk and the next one for all the tisues for each on the for variables with the corresponding values 
# brainomix neuros collaterals rank_transformation
sumstat_file="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/data/sumstats/rank_transformation.sumstats.gz"
weights_file="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/eQTL/GTExv8.EUR.Brain_Amygdala.pos" 
ref_ld_chr_file="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/1000G/LDREF/1000G.EUR."
weights_dir="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/eQTL/" 
out="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/rank_transformation/coloc/rank_transformation.GTExv8.EUR.Brain_Amygdala.dat.coloc"
jlim=FALSE
force_model=NA
coloc_P=1e-3
GWASN=8393671 
...
# Run TWAS and coloc for all chromosomes
```{r}
# Load in summary stats
sumstatfull = as.data.frame(fread(sumstat_file,head=T))

for(chr in 1:22){
# Select chromosome
sumstat = sumstatfull[sumstatfull$CHR==chr,]

# Load in list of weights
# TODO : TEST FOR NO HEADER HERE
wgtlist = read.table(weights_file,head=T,as.is=T)
wgtlist = wgtlist[ as.character(wgtlist$CHR) == as.character(chr) , ]
...
if ( !is.na(perm) && perm > 0 ) {
	out.tbl$PERM.PV = numeric(N)
	out.tbl$PERM.N = numeric(N)
	out.tbl$PERM.ANL_PV = numeric(N)
	permz = qnorm(perm_minp/2,lower.tail=F)
}


# Load in reference data
genos = read_plink(paste(ref_ld_chr_file,chr,sep=''),impute="avg")
...
for ( w in 1:nrow(wgtlist) ) {
	...
	# Load weights
	wgt.file = paste(weights_dir,"/",wgtlist$WGT[w],sep='')
	load(wgt.file)
	...
	if ( length(cur.Z) == 0 ) {
		...
		if ( !cur.FAIL ) {
			...
			if ( cur.twasr2pred > 0 ) {
				...
				if ( !is.na(perm) && perm > 0 && cur.twas^2 > permz^2 ) {
					perm.twas = rep(NA,perm)
					perm.pval = NA
					for ( i in 1:perm ) {
					...
					if ( is.na(perm.pval) ) {
						perm.pval = sum(perm.twas^2 > cur.twas[1]^2) / perm
						perm.N = perm
	...
	if ( cur.FAIL || sum(eqtlmod) == 0 || length(topeqtl) == 0 || is.na(topeqtl) ) {
		...
		if( caviar ) {
			...
			cav.out = paste( out , wgtlist$ID[w] , "CAVIAR" , sep='.' )
	...
	# perform COLOC test
	if ( !is.na(coloc_P) && !is.na(out.tbl$TWAS.Z[w]) && out.tbl$TWAS.P[w] < coloc_P && !is.na(wgtlist$N[w]) ) {
		b1 = wgt.matrix[,eqtlmod] / sqrt(wgtlist$N[w])
		snp1 = rownames(wgt.matrix) # Jara añade esta información antes era ocpiconal en coloc.abf, se asumía que estaba ordenado. Pero ahora chequean.
		b2 = cur.Z / sqrt(GWASN)
    snp2 = rownames(cur.LD) # Jara añade esta información antes era ocpiconal en coloc.abf, se asumía que estaba ordenado. Pero ahora chequean.
		vb1 = rep(1/wgtlist$N[w],length(b1))
		vb2 = rep(1/GWASN,length(b2))

		err = suppressMessages(capture.output(clc <- coloc.abf(dataset1=list(snp=snp1,beta=b1,varbeta=vb1,type="quant",N=wgtlist$N[w],sdY=1),dataset2=list(snp=snp2,beta=b2,varbeta=vb2,type="quant",N=GWASN,sdY=1))))
		# Jara añade las columnas de "snp", ahora necesarias para computar el análisis
...
if ( sum( mhc ) > 0 ) {
	...
	write.table( format( out.tbl[mhc,] , digits=3 ) , quote=F , row.names=F , sep='\t' , file=paste(out,".MHC.chr",chr,sep='') )
}
write.table( format( out.tbl[!mhc,] , digits=3 ) , quote=F , row.names=F , sep='\t' , file=paste0(out,".chr",chr) )
...
```{r}
#brainomix
files <- list.files(path="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/brainomix/coloc", pattern = "brainomix*", full.names = T)
twas_brainomix <- lapply(files, fread)
twas_brainomix <- ldply(twas_brainomix, data.frame)
twas_brainomix$TWAS.P_FDR <- p.adjust(twas_brainomix$TWAS.P, method = "fdr", n = nrow(twas_brainomix[!is.na(twas_brainomix$TWAS.P),]))

#neuros
files <- list.files(path="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/neuros/coloc", pattern = "neuros*", full.names = T)
twas_neuros <- lapply(files, fread)
twas_neuros <- ldply(twas_neuros, data.frame)
twas_neuros$TWAS.P_FDR <- p.adjust(twas_neuros$TWAS.P, method = "fdr", n = nrow(twas_neuros[!is.na(twas_neuros$TWAS.P),]))

#collaterals
files <- list.files(path="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/collaterals/coloc", pattern = "collaterals", full.names = T)
twas_collaterals <- lapply(files, fread)
twas_collaterals <- ldply(twas_collaterals, data.frame)
twas_collaterals$TWAS.P_FDR <- p.adjust(twas_collaterals$TWAS.P, method = "fdr", n = nrow(twas_collaterals[!is.na(twas_collaterals$TWAS.P),]))

#rank_transformation
files <- list.files(path="/home/stroke/Documentos/Miquel_Gallardo/real_data/TWAS/rank_transformation/coloc", pattern = "rank_transformation", full.names = T)
twas_rank_transformation <- lapply(files, fread)
twas_rank_transformation <- ldply(twas_rank_transformation, data.frame)
twas_rank_transformation$TWAS.P_FDR <- p.adjust(twas_rank_transformation$TWAS.P, method = "fdr", n = nrow(twas_rank_transformation[!is.na(twas_rank_transformation$TWAS.P),]))
```
